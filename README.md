...
This is a sandbox setup configuration for quickly getting started with OpenCode using Ollama hosted models. It provides a containerized development environment with everything needed to run OpenCode with local LLMs.
...
Supplied in the docker image is a form of an "entrypoint" that you can use directly, or indirectly. By default, the image is configured to enter `bash`, but you can set it to the script in [.scripts/opencode_start](.scripts/opencode_start).
...
**Via Docker exec:**
```bash
docker exec -it opencode-agent opencode_start
```
...
```bash
docker exec -it opencode-agent opencode_start --web
```

> If VS Code is actively connected to the devcontainer, it will auto-forward ports. Should you be running manually, you'll need to ensure the port is correct and matches the forward in the [docker-compose.yml](.devcontainer/opencode/docker-compose.yml) file (default: 4096).

This will start the web UI which you can access at `http://localhost:4096` (or your container's IP if running remotely).

#### Browser Control

Included in this project setup is [opencode-browser](https://github.com/different-ai/opencode-browser), a set of tooling to control the browser with a skill through `opencode`.
For more information, check out [.docs/opencode-browser.md](.docs/opencode-browser.md)!

#### TTS Capability

Included in this project setup is [speakturbo](https://github.com/EmZod/Speak-Turbo), a local-only TTS generator.
For more information, check out [.docs/opencode-browser.md](.docs/opencode-browser.md)!
...
* The docker image contains `llmfit`, a helpful utility to select models based on hardware and other criteria.
...
* TODO: Finish documenting the rest of the scripts found in `.scripts`
